      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid lg:grid-cols-[1fr_1.1fr] gap-6">
          ...
          <div className="space-y-6">
            ...
            <Card className="glass border-0 p-6 relative overflow-hidden">
              <div className="absolute -top-12 -right-12 h-40 w-40 rounded-full bg-gradient-primary opacity-20 blur-3xl pointer-events-none" />
              <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                <div>
                  <div className="flex items-center gap-2">
                    <Wand2 className="h-4 w-4 text-primary" />
                    <h3 className="font-semibold">AI weekly action plan</h3>
                    <Badge variant="outline" className="text-[10px] uppercase tracking-wider">Powered by AI</Badge>
                  </div>
                  <p className="text-sm text-muted-foreground mt-1">
                    Personalized to your inputs and last 7 days. Generated by Lovable AI.
                  </p>
                </div>
                <Button
              ...
              {aiPlan.isError && (
                <div className="rounded-xl p-4 bg-coral/10 border border-coral/30 text-sm">
                  <p className="font-medium text-coral">Couldn't generate plan</p>
                  <p className="text-muted-foreground mt-1">
                    {aiPlan.error?.message?.includes("429")
                      ? "Rate limit reached — please try again in a moment."
                      : aiPlan.error?.message?.includes("402")
                      ? "AI credits exhausted on this workspace. Add credits to continue."
                      : aiPlan.error?.message ?? "Unknown error"}
                  </p>
                </div>
              )}

              {!aiPlan.data && !aiPlan.isPending && !aiPlan.isError && (
                <div className="rounded-xl p-6 bg-muted/30 border border-dashed border-border/60 text-center">
                  <Sparkles className="h-6 w-6 text-primary mx-auto mb-2" />
                  <p className="text-sm text-muted-foreground">
                    Click <span className="font-medium text-foreground">Generate plan</span> to get 5–7 tailored weekly actions ranked by impact.
                  </p>
                </div>
              )}
              ...
              {aiPlan.data && (
                <div className="space-y-4">
                  <p className="text-sm leading-relaxed bg-primary/5 border border-primary/20 rounded-xl p-4">
                    {aiPlan.data.summary}
                  </p>
                  <div className="grid sm:grid-cols-2 gap-3">
                    {aiPlan.data.actions.map((a, i) => (
                      <div key={i} className="rounded-xl p-4 bg-muted/40 border border-border/50 hover:border-primary/40 transition">
